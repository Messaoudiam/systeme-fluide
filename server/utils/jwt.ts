...
const JWT_SECRET =
  ...
  "votre-cle-secrete-super-longue-et-complexe-pour-dev";
...
export function setJWTCookie(event: any, token: string) {
  setCookie(event, "auth-token", token, {
    ...
    sameSite: "lax", // Changé de 'strict' à 'lax' pour éviter les problèmes de refresh
...
export function clearJWTCookie(event: any) {
  deleteCookie(event, "auth-token", {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax", // Cohérence avec setJWTCookie
    path: "/", // Assure que le cookie est supprimé sur toutes les routes
    domain:
      process.env.NODE_ENV === "production" ? process.env.DOMAIN : undefined,
  });
}
